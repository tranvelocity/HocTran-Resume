<?php
  /**
   * Server side program to connect to DaaS stamp recognition server
   *
   * Deployment)
   *   root
...
echo 'this is digishot-api.php';
  ...
  //
  // Do not remove or overwrite other properties.
  //
  ...
  //
  //
  if($response === "recognize") { // Only when this value is set valid values for the application can received.
    ...
    if($resultCode === '1') { // Stamp is recognized successfully.($stampcode is valid)
      ...
      error_log(" userData = {$userData}");   // Not displayed unless set on client side
